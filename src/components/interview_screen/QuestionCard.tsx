  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {isActive ? (
          <span className="mt-1 inline-flex items-center rounded-full bg-primary-light px-2.5 py-1 text-[11px] font-semibold text-primary">
            Active
          </span>
        ) : null}
      </div>
      ...
      {evaluatedSummary && !isError && (
        <div className="mt-3 rounded-2xl bg-primary-light p-3">
          <div className="inline-flex items-center gap-2 rounded-full bg-primary-light px-2.5 py-1 text-[10px] font-semibold">
              ...
              {feedbackPoints.map((point, i) => {
                ...
                return (
                  <li key={i} className="flex items-start gap-2">
                    {isPositive ? (
                      <span className="mt-0.5 flex h-3.5 w-3.5 flex-shrink-0 items-center justify-center rounded-full bg-success">
                        <svg width="8" height="8" viewBox="0 0 8 8" fill="none">
                          <path d="M1.5 4L3 5.5L6.5 2" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      </span>
                    ) : (
                      <span className="mt-0.5 flex h-3.5 w-3.5 flex-shrink-0 items-center justify-center rounded-full bg-warning">
                        <svg width="8" height="8" viewBox="0 0 8 8" fill="none">
      ...
      {!isError && !evaluatedSummary && status && resolvedStatusText && (
        <div className="mt-2 flex items-center gap-2 rounded-lg bg-white px-2 py-1.5">
          {status === 'not-audible' ? (
            <span className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-orange-300 text-orange-500">
              !
            </span>
          ) : (
        ...
        <div className="mt-3 flex items-start justify-between gap-3 rounded-2xl bg-white px-3 py-2">
          ...
          {onReload && (
            <button
              type="button"
              onClick={onReload}
              className="mt-1 inline-flex items-center rounded-full bg-error-light px-2.5 py-1 text-[11px] font-semibold text-error-text hover:bg-red-200"
            >
              Reload Question
            </button>
          )}
